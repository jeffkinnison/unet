...
def downconv_block(i, filters, params=None, padding='same',
    ...
    c1 = Conv2D(filters, ks, padding=padding, data_format=data_format, **cp)(i)
    ...
    cp['activation'] = \
        get_activation(list(cp['activation'].keys())[0],
                       params=list(cp['activation'].values())[0])
    ks = cp.pop('kernel_size')
    c2 = Conv2D(filters, ks, padding=padding, data_format=data_format, **cp)(c1)
...
def unet(input_shape, data_format='channels_first', params=None):
    ...
    down3, c3 = downconv_block(down2, int(min_filters * 4),
                               params=params['down3'])
    down4, c4 = downconv_block(down3, int(min_filters * 8),
                               params=params['down4'])

    up1 = upconv_block(down4, int(min_filters * 16), params=params['up1'])
    crop1 = crop(c4, up1)
    merge_block = [up1, crop1]
    concat = concatenate(merge_block, axis=1)

    up2 = upconv_block(concat, int(min_filters * 8), params=params['up2'])
    crop2 = crop(c3, up2)
    merge_block = [up2, crop2]
    concat = concatenate(merge_block, axis=1)

    up3 = upconv_block(concat, int(min_filters * 4), params=params['up3'])
    crop3 = crop(c2, up3)
    merge_block = [up3, crop3]
    concat = concatenate(merge_block, axis=1)

    up4 = upconv_block(concat, int(min_filters * 2), params=params['up4'])
    crop4 = crop(c1, up4)
    merge_block = [up4, crop4]
    concat = concatenate(merge_block, axis=1)
    ...
    cp['activation'] = \
        get_activation(list(cp['activation'].keys())[0],
                       params=list(cp['activation'].values())[0])
    ks = cp.pop('kernel_size')
    out1 = Conv2D(min_filters, ks, padding='same', data_format=data_format,
                  **cp)(concat)
    ...
    cp['activation'] = \
        get_activation(list(cp['activation'].keys())[0],
                       params=list(cp['activation'].values())[0])
    ks = cp.pop('kernel_size')
    out2 = Conv2D(min_filters, ks, padding='same', data_format=data_format,
                  **cp)(out1)

    out3 = Conv2D(1, 1, activation='sigmoid', padding='same', data_format=data_format)(out2)
...
if __name__ == '__main__':
    ...
    params = {
        ...
        'down4': {
            'conv1': {'kernel_size': 3, 'activation': {'hard_sigmoid': 'hard_sigmoid'}, 'kernel_regularizer': 'l1', 'bias_regularizer': 'l2', 'activity_regularizer': 'l1_l2'},
            'conv2': {'kernel_size': 5, 'activation': {'leaky_relu': {'alpha': 0.2}}, 'kernel_initializer': 'he_normal', 'bias_initializer': 'he_uniform'}
        },
        'up1': {
        ...
        'up4': {
            ...
            'conv2': {'kernel_size': 5, 'activation': {None: None}, 'kernel_initializer': 'he_normal', 'bias_initializer': 'he_uniform'}
        },
        'out': {
            'conv1': {'kernel_size': 3, 'activation': {None: None}, 'kernel_regularizer': 'l1', 'bias_regularizer': 'l2', 'activity_regularizer': 'l1_l2'},
    ...
    model = unet((1, 256, 256), params=params)
